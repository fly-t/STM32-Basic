...
uint8_t U2_RxBuf[RX2_BUFLEN + 1]; // 接收数据缓冲区
uint32_t U2_RxBufWp;              // 接收缓存区写指针
uint32_t U2_RxBufRp;              // 接收缓存区读指针

uint8_t U2_TxBuf[TX2_BUFLEN + 1]; // 发送数据缓冲区
uint32_t U2_TxBufWp;              // 发送缓存区写指针
uint32_t U2_TxBufRp;              // 发送缓存区读指针

uint8_t U2_RevDataFlag;
uint8_t U2_Start1, U2_Start2, U2_End1, U2_End2;
...
void U2_printf(char *fmt, ...)
{
    va_list ap;
    va_start(ap, fmt);
    vsprintf((char *)U2_TxBuf, fmt, ap);
    va_end(ap);

    U2_Puts((uint8_t *)U2_TxBuf);
}

void Bsp_USART2_Init(void)
{
    UART_GPIO_Config(GPIOA, GPIO_PinSource2, GPIO_PinSource3, GPIO_Pin_2 | GPIO_Pin_3, GPIO_AF_USART2);
    UART_Config(UART_PORT, 9600, RCC_APB1Periph_USART2);
    NVIC_UART_Config(UART_PORT, USART2_IRQn, 4);
}
...
void USART2_IRQHandler(void)
{
    ...
    if (USART_GetITStatus(UART_PORT, USART_IT_RXNE) == SET)
    {
        ...
        U2_Rec_Data = USART_ReceiveData(UART_PORT);
        U2_Start1 = U2_Start2;
        U2_Start2 = U2_Rec_Data;

        if ((U2_Start1 == 0x7b) && (U2_Start2 == 0x7b))
        {
            U2_RevDataFlag = 1;
        }

        U2_RxBuf[U2_RxBufWp] = U2_Rec_Data; // 接收数据放入缓冲区。
        U2_RxBufWp = (U2_RxBufWp + 1) % RX2_BUFLEN;
        U2_End1 = U2_End2;
        U2_End2 = U2_Rec_Data;

        if ((U2_End1 == 0x7d) && (U2_End2 == 0x7d) && (U2_RevDataFlag == 1))
        {
