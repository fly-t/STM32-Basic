...
#include "bsp_uart8.h"
/*****************************************************/
#define UART_PORT UART8

uint8_t U8_RxBuf[RX8_BUFLEN + 1]; // 接收数据缓冲区
uint32_t U8_RxBufWp;              // 接收缓存区写指针
uint32_t U8_RxBufRp;              // 接收缓存区读指针

uint8_t U8_TxBuf[TX8_BUFLEN + 1]; // 发送数据缓冲区
uint32_t U8_TxBufWp;              // 发送缓存区写指针
uint32_t U8_TxBufRp;              // 发送缓存区读指针

uint8_t U8_RevDataFlag;
uint8_t U8_Start1, U8_Start2, U8_End1, U8_End2;
...
void U8_PutsNum(char *str, uint32_t Num)
{
    while (Num)
    {
        Num--;
...
void U8_printf(char *fmt, ...)
{
    va_list ap;
    va_start(ap, fmt);
    vsprintf((char *)U8_TxBuf, fmt, ap);
    va_end(ap);

    U8_Puts((char *)U8_TxBuf);
}

void Bsp_UART8_Init(void)
{
    UART_GPIO_Config(GPIOE, GPIO_PinSource1, GPIO_PinSource0, GPIO_Pin_0 | GPIO_Pin_1, GPIO_AF_UART8);
    UART_Config(UART_PORT, 9600, RCC_APB1Periph_UART8);
    NVIC_UART_Config(UART_PORT, UART8_IRQn, 4);
}
...
void UART8_IRQHandler(void)
{
    ...
    if (USART_GetITStatus(UART_PORT, USART_IT_RXNE) == SET)
    {
        ...
        U8_Rec_Data = USART_ReceiveData(UART_PORT);
        U8_Start1 = U8_Start2;
        U8_Start2 = U8_Rec_Data;

        if ((U8_Start1 == 0x7b) && (U8_Start2 == 0x7b))
        {
            U8_RevDataFlag = 1;
        }

        U8_RxBuf[U8_RxBufWp] = U8_Rec_Data; // 接收数据放入缓冲区。
        U8_RxBufWp = (U8_RxBufWp + 1) % RX8_BUFLEN;
        U8_End1 = U8_End2;
        U8_End2 = U8_Rec_Data;

        if ((U8_End1 == 0x7d) && (U8_End2 == 0x7d) && (U8_RevDataFlag == 1))
        {
