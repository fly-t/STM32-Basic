...
uint8_t U1_RxBuf[RX1_BUFLEN + 1]; // 接收数据缓冲区
uint32_t U1_RxBufWp;              // 接收缓存区写指针
uint32_t U1_RxBufRp;              // 接收缓存区读指针

uint8_t U1_TxBuf[TX1_BUFLEN + 1]; // 发送数据缓冲区
uint32_t U1_TxBufWp;              // 发送缓存区写指针
uint32_t U1_TxBufRp;              // 发送缓存区读指针

uint8_t U1_RevDataFlag;
uint8_t U1_Start1, U1_Start2, U1_End1, U1_End2;
...
{
    // 启用UART的时钟
    RCC_APB2PeriphClockCmd(RCC_APB1Periph_UARTx, ENABLE);
...
void U1_PutsNum(char *str, uint32_t Num)
{
    while (Num)
    {
        Num--;
...
void U1_printf(char *fmt, ...)
{
    va_list ap;
    va_start(ap, fmt);
    vsprintf((char *)U1_TxBuf, fmt, ap);
    va_end(ap);

    U1_Puts((char *)U1_TxBuf);
}

void Bsp_USART1_Init(void)
{
    UART_GPIO_Config(GPIOA, GPIO_PinSource9, GPIO_PinSource10, GPIO_Pin_9 | GPIO_Pin_10, GPIO_AF_USART1);
    UART_Config(UART_PORT, 9600, RCC_APB2Periph_USART1);
    NVIC_UART_Config(UART_PORT, USART1_IRQn, 4);
}
...
void USART1_IRQHandler(void)
{
    ...
    if (USART_GetITStatus(UART_PORT, USART_IT_RXNE) == SET)
    {
        ...
        U1_Rec_Data = USART_ReceiveData(UART_PORT);
        U1_Start1 = U1_Start2;
        U1_Start2 = U1_Rec_Data;

        if ((U1_Start1 == 0x7b) && (U1_Start2 == 0x7b))
        {
            U1_RevDataFlag = 1;
        }

        U1_RxBuf[U1_RxBufWp] = U1_Rec_Data; // 接收数据放入缓冲区。
        U1_RxBufWp = (U1_RxBufWp + 1) % RX1_BUFLEN;
        U1_End1 = U1_End2;
        U1_End2 = U1_Rec_Data;

        if ((U1_End1 == 0x7d) && (U1_End2 == 0x7d) && (U1_RevDataFlag == 1))
        {
